export class StartMenu extends Phaser.Scene {
	...
	preload() {
		...
		this.load.spritesheet("bear", "assets/Bear.png", { frameWidth: 64, frameHeight: 64 })
...
//		this.load.spritesheet("srd", "assets/Sword.png", { frameWidth: 32, frameHeight: 32 })
	}
	...
	create() {
		...
		this.anims.create({
			key: "lld",
			frames: this.anims.generateFrameNumbers("lcd", { start: 0, end: 7, first: 0 }),
			framerate: 0,
			repeat: -1,
			duration: 750
		})
		...
		this.anims.create({
			key: "ber",
			frames: this.anims.generateFrameNumbers("bear", { start: 0, end: 5, first: 0 }),
			framerate: 0,
			repeat: -1,
			duration: 1000
		})
/*				this.anims.create({
			key: "slice",
			frames: this.anims.generateFrameNumbers("srd", { start: 0, end: 5, first: 0 }),
			framerate: 0,
			repeat: -1,
			duration: 1000
		})
	*/
		...
		this.add.sprite(150, 300, "bear").play("ber")		
	//	this.add.sprite(300, 300, "srd").play("slice")
	}
	update() {
